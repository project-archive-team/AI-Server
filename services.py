...
CAREER_ROLE_ALIGNMENT_RULE = (
    "답변 전반은 지원 직무와 프로젝트 자료에서 확인되는 지원자 본인의 담당 역할(AI, 백엔드, "
    "프론트엔드 등)을 기준으로 구성합니다. 지원 직무와 담당 역할이 직접 연결되는 경험과 역량을 "
    "우선 선택하고, 프로젝트 전체 성과보다 본인이 맡은 책임, 판단, 행동, 기술적 기여를 중심으로 "
...
4. {CAREER_ROLE_ALIGNMENT_RULE}
5. STAR 각 항목은 서로 중복하지 않고 지원자 관점의 한국어 문장으로 작성합니다.
6. 질문에 답하는 데 필요한 역할이나 성과 수치가 자료에 없으면 추측하지 말고 missingEvidence에 구체적으로 적습니다.
7. finalAnswer는 STAR 내용을 자연스럽게 연결한 완성형 답변이며 새로운 사실을 추가하지 않습니다.
8. 출처나 인용 정보는 출력하지 않습니다. 인용은 서버가 별도로 결합합니다.
9. {INTERVIEW_SPOKEN_ANSWER_RULE}
""".strip()
    prompt = f"""
[지원 직무]
{job_role}
...
3. {CAREER_ROLE_ALIGNMENT_RULE}
4. likelihood는 HIGH, MEDIUM, LOW 중 하나만 사용합니다.
5. modelAnswer는 지원자가 실제 면접에서 말할 수 있는 한국어 답변으로 작성하며, 반드시 다음 STAR 구조를 따릅니다.
   S: 직무 역량을 보여주는 데 필요한 상황과 배경만 간결하게 작성합니다.
   T: 팀 전체의 목표보다 지원자 본인이 맡은 과제, 책임과 해결해야 했던 문제를 중심으로 작성합니다. 다만 지원자의 역할을 설명하는 데 필요한 경우 팀 전체의 목표를 함께 작성할 수 있습니다.
   A: 지원자가 직접 내린 판단과 수행한 행동, 기술적 선택 및 기여를 구체적으로 작성합니다. 팀이 수행한 일과 지원자 개인이 수행한 일을 명확히 구분합니다.
   R: 프로젝트 자료에서 확인할 수 있는 성과와 변화를 작성합니다. 단순한 업무 수행을 성과로 포장하지 않으며, 근거가 없는 수치나 결과를 추측하지 않습니다.
   각 항목은 반드시 S:, T:, A:, R: 라벨로 시작하고 항목마다 줄을 바꿔 작성합니다.
6. 근거가 부족한 내용은 아는 척하지 말고 확인할 수 없다고 명시합니다.
7. checkpoints는 면접관이 평가할 핵심 요소를 2~4개 작성합니다.
8. followUps는 질문마다 정확히 2개 만들고, 각 질문에 근거 기반 recommendedAnswer를 제공합니다.
9. 추천 답변에도 원문에 없는 사실이나 수치를 추가하지 않습니다.
10. 출처나 인용 정보는 출력하지 않습니다. 인용은 서버가 별도로 결합합니다.
11. {INTERVIEW_SPOKEN_ANSWER_RULE}
""".strip()
    prompt = f"""
[지원 직무]
{job_role}
